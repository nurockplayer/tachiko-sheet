...
At a stable Final Candidate, use a fresh independent exact-HEAD **Oracle** review. The default execution transport is the qualified **ego-lite → Oracle** path; transport is configuration, not authority. Run deterministic and required hosted gates first. For ordinary material final review, GPT-5.6 Sol **High** is the default effort. **Extra High** is reserved for explicitly high/critical-risk review when the owning Issue/lead records why the higher tier is required. Verify the requested/effective model and effort fail-closed before granting review credit; unknown provenance or transport/session failure is REVIEW_INCOMPLETE, not PASS. Oracle is read-only and must not have participated in the candidate's design, implementation or acceptance/evidence preparation. Required merge verdict: `No blocking findings.` Oracle does not promote Ready, clear HOLD, write code or merge. GPT-6 Pro is separately authorized architecture/judgment capacity, not an implicit Oracle fallback. A material commit after the receipt invalidates it.
