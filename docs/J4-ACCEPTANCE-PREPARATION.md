...
- Upstream provenance-only canary: `tachiko-work@518aaa55` at
  `apps/designer/e2e/fixtures/keyed-grouped-sum-v1.roproj/`. It is an ordinary
  two-table project; any Sheet fixture must preserve those source bytes and
  record per-file hashes. It is not a new Sheet codec or production authoring
  path.
...
| --- | --- | --- |
| Normal entry | User opens the preserved two-table canary through normal Sheet Open, selects Catalog/Sales and visible field names. | Browser trace; no IDs/JSON control. |
| Authoritative result | Core result is line `600/1000/200`, groups `PEN=800`, `NOTE=1000`, total `1800`. | Runtime/browser assertion reads core projection; no JavaScript aggregation. |
| Currentness | Edit a relevant Catalog/Sales value through the existing core edit path; refresh the definition query. | New revision and authoritative changed result, never cached old success. |
