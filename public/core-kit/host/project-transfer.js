...
export async function projectTransferFromFiles(files) {
    if (files.length === 0)
        throw new Error("No project directory was selected.");
 ...
 */
export function projectTransferFromEntries(entries) {
...
export function projectTransferToEntries(input) {
    ...
    for (let index = 0; index < count; index += 1) {
        if (offset + 6 > bytes.byteLength)
            throw new Error("The canonical project transfer is truncated.");
        ...
        offset += pathLength;
        if (!isSafeRelativePath(path) || paths.has(path))
            throw new Error("The canonical project transfer has invalid paths.");
        paths.add(path);
        entries.push({ path, bytes: bytes.slice(offset, end).buffer });
        offset = end;
    }
    if (offset !== bytes.byteLength)
        throw new Error("The canonical project transfer has trailing bytes.");
    return entries;
}
function encodeTransfer(transferred, total) {
    const output = new Uint8Array(total);
    const view = new DataView(output.buffer);
    let offset = 0;
    output.set(PROJECT_BUNDLE_MAGIC, offset);
...
function relativeFilePath(file, expectedRoot) {
