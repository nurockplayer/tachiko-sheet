...
For architecture-related work, read [`docs/architecture/README.md`](docs/architecture/README.md), [`clean-code.md`](docs/architecture/clean-code.md), and [`terra-playbook.md`](docs/architecture/terra-playbook.md), tracked in [#33](https://github.com/nurockplayer/tachiko-sheet/issues/33). Respect their proposal/adoption status. They describe Sheet-local boundaries and bounded migration, not a blanket source-refactor dispatch or Ready decision. Follow live #1's mandatory Astra and conditional Pro triggers before freezing a decision or assigning implementation; obtain required Astra advice before freeze/assignment and required Pro advice before implementation, recording HOLD with the exact unresolved question if required Pro is unavailable. Outside those binding triggers, Astra advice is optional. Independent Oracle final review, active writer ownership, and unresolved product/acceptance HOLDs remain unchanged. Read the relevant sections at intake or a material architecture change, not every unchanged heartbeat.
