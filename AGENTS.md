...
At a stable Final Candidate, use one eligible fresh independent exact-HEAD **Oracle** review through **ego-lite** by default. Oracle is read-only and must be independent of the candidate's design, implementation and acceptance/evidence preparation. Run deterministic/hosted gates first. GPT-5.6 Sol **High** is the default effort for ordinary material final review; use **Extra High** only for an explicitly high/critical-risk candidate with the reason recorded on the owning Issue/PR. Verify requested/effective model and effort before granting review credit; unknown provenance or transport failure fails closed. Required merge verdict is `No blocking findings.` A material commit after review invalidates the receipt. GPT-6 Pro is separately authorized judgment capacity, never a silent Oracle fallback. After all exact-head gates and the Oracle receipt pass, Sol/Conductor performs the protected merge.
