...
const sheet = (columns, rows) => `<?xml version="1.0" encoding="utf-8"?><x:worksheet xmlns:x="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><x:sheetData>${[[...columns], ...rows].map((row, rowIndex) => `<x:row r="${rowIndex + 1}">${row.map((value, columnIndex) => `<x:c r="${String.fromCharCode(65 + columnIndex)}${rowIndex + 1}" t="str"><x:v>${value}</x:v></x:c>`).join("")}</x:row>`).join("")}</x:sheetData></x:worksheet>`;
...
try {
  ...
  assert.match(await groups(page), /PEN: 800/);
